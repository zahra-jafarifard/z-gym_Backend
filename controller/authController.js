...
exports.postRegister = (req , res , next)=>{
    const {name , lastName , mobile , password , repaetPassword , gender , birthDay , height , weight} = req.body;
    ...
    .then(user => {
        ...
        .then(matchPass => {
            if (!matchPass){
               ...
               if (rememberMe){
                 token = jwt.sign({mobile: user.mobile , userId:user.id}
...
exports.postResetPassword = (req , res , next)=>{
    ...
    .then(() => {
        ...
        res.status(200).json({resetToken:resetPassToken , rndNumber:rndNumber});
    })
    .catch(e => {
        next(new httpError(e , 500))
    })
...
exports.postCreateMember = (req , res , next) => {
    const{name , lastName , mobile,password, birthDay, weight, height, gender , status , group} =req.body;
...
exports.postUpdateMember = (req ,res ,next) => {
    let {name , lastName , mobile , status ,birthDay , weight , height , gender , group}=req.body;
